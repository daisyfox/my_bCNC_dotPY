...
class Application(Toplevel):
	def __init__(self, master, **kw):
		...
		paned.add(panedframe, minsize=600)
...
#		self.bind('<F2>',		self.rename)
	...
	def _controlTab(self):
		frame = self.tabPage["Control"]
                                ...
                                # copied from 'Main Frame' above (Main Frame create Paned window for canvas & notebook)
		pane = PanedWindow(frame, orient=HORIZONTAL)
		pane.pack(fill=BOTH, expand=YES)

                                # copied from 'Tabbed Notebook' above (frame added for Notebook)
		paneframe = Frame(pane)
		pane.add(paneframe, minsize=300)
		frame = paneframe

		# Control -> Status
		lframe = LabelFrame(frame, text="Status", foreground="DarkBlue")
		lframe.pack(side=TOP, fill=X)

		row = 0
		col = 0
		Label(lframe,text="Status:").grid(row=row,column=col,sticky=E)
		col += 1
		self.state = Label(lframe, text=NOT_CONNECTED,
				background=STATECOLOR[NOT_CONNECTED])
		self.state.grid(row=row,column=col, columnspan=3, sticky=EW)
		...
		# work
		col += 1
		self.xwork = Label(lframe, background="White",anchor=E)
		self.xwork.grid(row=row,column=col,padx=1,sticky=EW)
		tkExtra.Balloon.set(self.xwork, "X work position")

		# ---
		col += 1
		self.ywork = Label(lframe, background="White",anchor=E)
		self.ywork.grid(row=row,column=col,padx=1,sticky=EW)
		tkExtra.Balloon.set(self.ywork, "Y work position")

		# ---
		col += 1
		self.zwork = Label(lframe, background="White", anchor=E)
		self.zwork.grid(row=row,column=col,padx=1,sticky=EW)
		tkExtra.Balloon.set(self.zwork, "Z work position")
		...
		col += 1
		self.xmachine = Label(lframe, background="White",anchor=E)
		self.xmachine.grid(row=row,column=col,padx=1,sticky=EW)

		col += 1
		self.ymachine = Label(lframe, background="White",anchor=E)
		self.ymachine.grid(row=row,column=col,padx=1,sticky=EW)

		col += 1
		self.zmachine = Label(lframe, background="White", anchor=E)
		self.zmachine.grid(row=row,column=col,padx=1,sticky=EW)
		...
		col += 1
		self.step = tkExtra.Combobox(lframe, width=2, background="White")
		...
		lframe.grid_columnconfigure(6,weight=1)
                                ...
                                # Control -> User
		paneframe = Frame(pane)
		pane.add(paneframe)  #, minsize=200)
		frame = paneframe
		...
		# Control -> Check Run
		lframe = LabelFrame(frame, text="Check Run", foreground="DarkBlue")
		lframe.pack(side=TOP, fill=X)
		f = Frame(lframe)
		f.pack(side=TOP,fill=X)
		b = Button(f, text="Run gCode in test mode",
				compound=LEFT,
		...
		# Control -> Live Run
		lframe = LabelFrame(frame, text="Live Run", foreground="DarkBlue")
		lframe.pack(side=TOP, fill=X)
		f = Frame(lframe)
		f.pack(side=TOP,fill=X)
		b = Button(f, text="Run",
	...
	#----------------------------------------------------------------------
	# GRBL CheckMode Results
	#----------------------------------------------------------------------
	def _checkerTab(self):
		frame = self.tabPage["Checker"]
		self.checker = Text(frame,
					background="White",
					width=20,
					wrap=NONE,
					state=DISABLED)
		self.checker.pack(side=LEFT, fill=BOTH, expand=YES)
		sb = Scrollbar(frame, orient=VERTICAL, command=self.checker.yview)
		sb.pack(side=RIGHT, fill=Y)
		self.checker.config(yscrollcommand=sb.set)
		self.checker.tag_config("ERROR",  foreground="Red")
...
###		self.editor.replaceDialog()
	...
	def commandExecute(self, addHistory=True):
		...
		if line =="$C":
			self._checkMode = not self._checkMode
...
#			self.statusbar["text"] = "Height: %g  Depth-per-pass: %g  Feed: %g"%(self.height,self.depth_pass, self.feed)
	...
	def checkGcode(self):
		if not self._checkMode:
			self.sendGrbl("$C\n")
		self.tabPage.changePage("Terminal")
		self.after(1000, self.checkingGcode) #fudge

	def checkingGcode(self):
		self.checkedGcode = [[], []]
		self._checkMode = True
		self.run()
	...
	def runEnded(self):
		if self._checkMode:
			self._checkMode = False
			self.queue.put("$C\n")
			self.checker["state"] = NORMAL
			self.checker.delete("1.0", END)
			for snt, res in zip(self.checkedGcode[0], self.checkedGcode[1]):
				if res.find("ok") >= 0: self.checker.insert(END, res + "  :  " + snt + "\n")
				else: self.checker.insert(END, res + "  :  " + snt + "\n", "ERROR")
			self.checker["state"] = DISABLED
		...
		while self.log.qsize()>0 and time.time()-t<0.1:
			try:
				...
				if io:
					...
					if self._checkMode: self.checkedGcode[0].append(line.strip())
				else:
					self.terminal.insert(END, line)
					if self._checkMode: self.checkedGcode[1].append(line.strip())
			except Empty:
				break
